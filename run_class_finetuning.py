...
def get_args():
    ...
    parser.add_argument(
        '--linear_probe', action='store_true', default=False)
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            'HMDB51', 'Diving48', 'Kinetics-710', 'MIT', 'CAG'
        ],
        type=str,
        help='dataset')
    parser.add_argument(
...
def main(args, ds_init):
    ...
    if args.validation:
        ...
        print(
            f"{len(dataset_val)} val images: Precision {test_stats['precision']:.2f}%, Recall {test_stats['recall']:.2f}%, loss {test_stats['loss']:.4f}"
        )
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            test_stats = validation_one_epoch(data_loader_val, model, device)
            print(
                f"Precision of the network on the {len(dataset_val)} val images: {test_stats['precision']:.2f}%"
            )
            print(
                f"Recall of the network on the {len(dataset_val)} val images: {test_stats['recall']:.2f}%"
            )

            if log_writer is not None:
                log_writer.update(
                    val_precision=test_stats['precision'], head="perf", step=epoch)
                log_writer.update(
                    val_recall=test_stats['recall'], head="perf", step=epoch)
                log_writer.update(
...
if __name__ == '__main__':
    ...
    if opts.cross_val:
        ...
        import pandas as pd
        data_dir = Path(opts.data_path)
        df = pd.read_csv(data_dir / 'finetune.csv', sep='\t')
        skf = StratifiedKFold(n_splits=4, shuffle=True, random_state=seed)
        for i, (train_index, val_index) in enumerate(skf.split(df, df['label'])):
            # if i != 1:
            #     continue
            args = copy.deepcopy(opts)
            if utils.is_main_process():
                df_train, df_val = df.iloc[train_index], df.iloc[val_index]
                df_train.to_csv(data_dir / f'train.csv', sep='\t', index=False)
                df_val.to_csv(data_dir / f'val.csv', sep='\t', index=False)
            torch.distributed.barrier()
            args.output_dir = os.path.join(args.output_dir, f'fold_{i}')
            args.log_dir = os.path.join(args.log_dir, f'fold_{i}')
            main(args, ds_init)
    else:
        main(opts, ds_init)
