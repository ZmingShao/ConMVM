...
class ShiftVideo(object):
    """
    Args:

    """

    def __init__(self, pixel=31, frame=4, size=224, clip_len=16):
    ...
    def __call__(self, clip: List[Image.Image]) -> List[Image.Image]:
        ...
        frame = random.randint(0,self.frame)
        
        assert frame+self.clip_len<=len(clip)
        
        shifted_clip = []
        for i in range(frame, frame+self.clip_len):
            img = np.array(clip[i])
            h, w, _ = img.shape
            assert pixel_h+self.size<=h
            assert pixel_w+self.size<=w
            img = img[0+pixel_h:pixel_h+self.size,0+pixel_w:pixel_w+self.size,:]
            # return Image.fromarray(img.astype(np.uint8))
            shifted_clip.append(Image.fromarray(img.astype(np.uint8)))
...
class GaussianBlur(object):
    ...
    def __call__(self, clip: List[Image.Image]) -> List[Image.Image]:
        ...
        return [img.filter(ImageFilter.GaussianBlur(radius=radius)) for img in clip]
...
class RandomApply(object):
    ...
    def __call__(self, clip: List[Image.Image]) -> List[Image.Image]:
        ...
        if self.random_apply():
            return self.transforms(clip)  # type: ignore
        else:
            return clip
