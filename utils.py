...
def save_model(args,
    ...
    if loss_scaler is not None:
        ...
        for checkpoint_path in checkpoint_paths:
            to_save = {
                'model': (model_without_ddp.encoder if encoder_only else model_without_ddp).state_dict(),
...
def auto_load_model(args,
    ...
    ckpt_latest = output_dir / 'checkpoint-latest.pth'
    if args.auto_resume and len(args.resume) == 0 and ckpt_latest.exists():
        args.resume = str(ckpt_latest)
...
def multiple_pretrain_samples_collate(batch, fold=False):
    ...
    Returns:
        (tuple): collated data batch.
    """
    process_data, process_data_t, encoder_mask, decoder_mask = zip(*batch)

    process_data = [item for sublist in process_data for item in sublist]
    process_data_t = [item for sublist in process_data_t for item in sublist]
    ...
    process_data, process_data_t, encoder_mask, decoder_mask = (
        default_collate(process_data), 
        default_collate(process_data_t),
        default_collate(encoder_mask),
        default_collate(decoder_mask),
    )
    if fold:
        return [process_data], [process_data_t], encoder_mask, decoder_mask
    else:
        return process_data, process_data_t, encoder_mask, decoder_mask
