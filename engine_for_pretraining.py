...
def train_one_epoch(model: torch.nn.Module,
    ...
    for step, batch in enumerate(
        ...
        images, images_t, bool_masked_pos, decode_masked_pos = batch
        ...
        if loss_scaler is None:
            loss_rc, loss_ct = model(images, images_t, labels, bool_masked_pos, decode_masked_pos)
        else:
            with torch.cuda.amp.autocast():
                loss_rc, loss_ct = model(images, images_t, labels, bool_masked_pos, decode_masked_pos)

        loss = loss_weight * loss_rc + (1 - loss_weight) * loss_ct
        ...
        if log_writer is not None:
            ...
            log_writer.update(loss_rc=loss_rc.item(), head="loss")
            log_writer.update(loss_ct=loss_ct.item(), head="loss")
